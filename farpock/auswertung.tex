...
Messreihe 1 wurde bei $2010 \pm 7 Hz$ Sinussignal aufgenommen, Messreihe 2 bei $1800 \pm 1 Hz$ bzw $1815 \pm 1 Hz$ aufgenommen.
...
def sinus_auswertung(u_plus, u_minus):
  ...
  for i in range(len(u_plus)):
    print repr(u_plus[i]) + r' & ' + repr(-u_minus[i]) + r' & ' + repr(u_halb[i]) + r' \\ '
...
def auswertung_faraday(I, s_I, alpha, s_alpha):
  ...
  print r'\begin{tabular}{ll} \toprule'
  print r' I [A] & $\alpha$ [$\circ$] \\'
  print r' \midrule '

  for i in range(len(I)):
    print repr(I[i]) + r' & ' + repr(alpha[i]) + r' \\ '
